		...
		Steps: []resource.TestStep{
			{
				...
				Config: testCertificateBasic(localName, name, certificateData),
			},
		},
	})
}

func testCertificateBasic(localName string, name string, certificateData string) string {
	return fmt.Sprintf(`resource "octopusdeploy_certificate" "%s" {
		name = "%s"
		certificate_data = "%s"
	}`, localName, name, certificateData)
}
