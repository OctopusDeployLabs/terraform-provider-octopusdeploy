...
func (suite *IntegrationTestSuite) TestTenantProjectResource_UpgradeFromSDK_ToPluginFramework() {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ProtoV6ProviderFactories: ProtoV6ProviderFactories(),
				Config:                   updatedTenantConfig(name),
				Check: resource.ComposeTestCheckFunc(
					testTenantUpdated(t, name),
				),
			},
			{
				ProtoV6ProviderFactories: ProtoV6ProviderFactories(),
...
func updatedTenantConfig(name string) string {
	return fmt.Sprintf(`
	resource "octopusdeploy_tenant" "tenant1" {
		name        = "tenant %[1]s"
...
func testTenantUpdated(t *testing.T, name string) resource.TestCheckFunc {
