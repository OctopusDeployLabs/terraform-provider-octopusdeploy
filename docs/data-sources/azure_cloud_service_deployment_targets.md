...
- **cloud_service_name** (String, Read-only)
- **default_worker_pool_id** (String, Read-only)
- **endpoint** (List of Object, Read-only) (see [below for nested schema](#nestedatt--azure_cloud_service_deployment_targets--endpoint))
...
- **swap_if_possible** (Boolean, Read-only)
...
- **use_current_instance_count** (Boolean, Read-only)
