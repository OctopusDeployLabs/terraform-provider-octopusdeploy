# Create an lifecycle with Terraform

The **lifecycle Creation** directory contains the following files:
...
The `main.tf` configuration file has two blocks, one for the provider and another for the `lifecycle` resource. The provider calls for three inputs:
...
The resource itself that's being created is done with the `octopusdeploy_lifecycle` Terraform resource. The values in the input blocks are:
...
The `variables.tf` configuration file contains the following variables to be used in the `main.tf` configuration file.
  ...
  - lifecycleName: The name of the new lifecycle
